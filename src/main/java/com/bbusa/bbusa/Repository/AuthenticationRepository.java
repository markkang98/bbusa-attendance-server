...
public interface AuthenticationRepository extends JpaRepository<AuthenticationEntity, String> {
    @Query(value = "SELECT salt FROM trial_database.authentication x WHERE x.user_id = :user_id", nativeQuery = true)
    byte [] getSalt(@Param("user_id")String user_id);

    @Query(value = "SELECT hashed_password FROM trial_database.authentication x WHERE x.user_id = :user_id", nativeQuery = true)
